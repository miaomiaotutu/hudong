...
- (void)initValues
{
    ...
    UILabel *line=[[UILabel alloc]initWithFrame:CGRectMake(0, _topScrollView.frame.size.height-0.5f, 2000, 0.5f)];
//    [line setBackgroundColor:[UIColor colorWithRed:<#(CGFloat)#> green:<#(CGFloat)#> blue:<#(CGFloat)#> alpha:<#(CGFloat)#>]];
        ...
        if (self.rigthSideButton.bounds.size.width > 0) {
           // _rigthSideButton.frame = CGRectMake(self.bounds.size.width - self.rigthSideButton.bounds.size.width, 0,
            ...
            _topScrollView.frame = CGRectMake(0, 0,
                                              self.bounds.size.width , kHeightOfTopScrollView);
...
//                                   lineBreakMode:NSLineBreakByTruncatingTail];
        CGSize textSize=CGSizeMake(self.bounds.size.width/3, kHeightOfTopScrollView) ;
        //累计每个tab文字的长度
        ...
        [button setTag:i+100];
        if (i == 0) {
            _shadowImageView.frame = CGRectMake(kWidthOfButtonMargin, 0, textSize.width, kHeightOfTopScrollView);
            button.selected = YES;
        }
